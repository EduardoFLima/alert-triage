...
own identifier has no address template, the system SHALL link to the thing that
...
where the platform's address template allows a period to be expressed, so that
a reader following it days later sees the alert rather than the present.
...
#### Scenario: The alert's own identifier has no address template
