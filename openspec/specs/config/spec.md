...
how it groups, how it investigates, which of its services are critical.
Settings that describe how to *reach* an external platform — endpoints, sites,
regions, hostnames, credentials — SHALL NOT be read from `config.yaml`, and
SHALL be resolved from the environment only. A config file is therefore
portable across deployments of the same behavior, and carries nothing that
changes when the same triage behavior points at a different account or region.

#### Scenario: Connection setting present in the config file
- **WHEN** `config.yaml` contains a section describing a platform endpoint,
  site, or credential
- **THEN** the system refuses to start and names that section, rather than
  reaching the platform with it or starting with it silently unread
