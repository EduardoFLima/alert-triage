...
- [ ] 1.1 Delete the `critical_services` tests first, so the suite stops
...
- [ ] 1.2 Delete `CriticalService` from `configuration/settings.py`,
  `Config.critical_services` from `configuration/port.py`, and
  `_critical_services` and its imports from the YAML loader
- [ ] 1.3 Drop the `critical_services` config stubs from the four
  `tests/unit/app/` run tests
- [ ] 1.4 Remove the `critical_services` block from `config.example.yaml` and
  from `config.yaml`
- [ ] 1.5 Confirm `critical_services` is now an unknown key: a test showing
  the loader refuses a config that still sets it, naming the key

## 2. Scope grows a services mapping (file only)

- [ ] 2.1 Failing test: a config naming services under `scope.services`
  resolves them, each with a name and a `critical` flag defaulting to false
- [ ] 2.2 Add `ServiceScope` and `Scope.services` to
  `configuration/settings.py`; make `Scope.owner` optional
- [ ] 2.3 Failing test: `critical: true` on an entry resolves as critical, and
  an entry written with no settings resolves as in-scope and not critical
- [ ] 2.4 Read the mapping in the loader, refusing an entry that is not a
  mapping of setting keys, and refusing an unknown key within one
- [ ] 2.5 Failing test: services alone satisfy scope — the run starts with no
  owner resolved from either source
- [ ] 2.6 Failing test: neither key resolving refuses the run, with a message
  naming owner, services, or both
- [ ] 2.7 Failing test: `scope.services: {}` with no owner refuses the run,
  as though the key were absent — the case a naive check gets wrong
- [ ] 2.8 Enforce "at least one" in the loader's `_scope`, raising
  `ConfigError` (see design — deliberately not in `Scope.__post_init__`)
- [ ] 2.9 Expose `Config.scope` unchanged in shape; confirm no consumer of
  `scope.owner` broke on it becoming optional

## 3. The environment declares the services

- [ ] 3.1 Failing test: `SCOPE_SERVICES=a,b` with no config file resolves
  exactly those two services, neither critical
- [ ] 3.2 Teach the loader to read a comma-separated set of names — its first
  mapping-valued environment override
- [ ] 3.3 Failing test: `SCOPE_SERVICES` replaces the file's section entirely;
  a service the file declared critical is neither in scope nor critical when
  the variable does not name it
- [ ] 3.4 Failing test: `SCOPE_SERVICES_<NAME>_CRITICAL` sets criticality on
  an entry, whether the set came from the file or from `SCOPE_SERVICES`
- [ ] 3.5 Failing test: with `SCOPE_SERVICES` unset, the file's services and
  their settings stand

## 4. Ingestion filters on both

- [ ] 4.1 Failing test: services alone bound the fetch — alerts for the named
  services are returned regardless of owner
- [ ] 4.2 Failing test: both filters resolved means an alert must satisfy
  both; an alert for a named service owned by someone else is not returned
- [ ] 4.3 Failing test: no alert satisfying both is an empty result, not a
  failure
- [ ] 4.4 Compose the service term into the query in
  `triage/adapters/datadog/alert_source.py`, alongside `team:`; settle the
  grouped-vs-OR'd form (design — Open Questions)
- [ ] 4.5 Pass the resolved services from `app/composition.py` into the
  adapter beside the owner
- [ ] 4.6 Failing test: a critical service is fetched on the same terms as any
  other — criticality reaches nothing here

## 5. Criticality reaches the investigation

- [ ] 5.1 Failing test: `InvestigationTarget` states criticality and defaults
  to not critical, so existing three-field callers keep working
- [ ] 5.2 Add the field to `investigation/contract.py` and one line to
  `describe()` — which `_brief` is built on, so it reaches the diagnostician
  and the report writer from one place
- [ ] 5.3 Failing test: `Incident.investigation_target()` carries criticality
  through from the resolved scope
- [ ] 5.4 Thread the resolved services to wherever the incident builds its
  target, without the investigation context reading config of its own
- [ ] 5.5 Failing test: an investigation reads criticality only from its
  target and consults no configuration
- [ ] 5.6 Failing test: the written account identifies a critical service's
  incident as critical
- [ ] 5.7 Failing test: two incidents differing only in criticality get the
  same specialists under the same bounds

## 6. Documentation

- [ ] 6.1 Rewrite slice 11 in `docs/vision.md` — scoping by service and
  criticality, replacing the escalation path
- [ ] 6.2 Remove escalation from the other five sites in `docs/vision.md`: the
...
- [ ] 6.3 Restate slice 12: a tripped breaker produces a partial report marked
  incomplete, and nothing escalates
- [ ] 6.4 Document `scope.services` in `config.example.yaml` — stating at the
  key itself that naming any service narrows triage to the named services
- [ ] 6.5 Update `docs/configuration.md` and `README.md` for the new scope rule
