...
- [ ] 1.1 Write failing tests for a `TriageReport` carrying the incident it
      ...
      channel-neutral)
- [ ] 1.2 Implement `domain/report.py` to pass them, stdlib-only and depending
      on nothing beyond `Incident`
- [ ] 1.3 Write a failing test that a `Notifier` implementation is recognised
...
- [ ] 1.4 Implement `ports/notifier.py`: a synchronous `deliver` taking one
...
- [ ] 2.1 Write a failing test that rendering a report produces a message whose
      ...
      Email delivery)
- [ ] 2.2 Implement the pure rendering half in `adapters/email/`, using
      `email.message.EmailMessage`
- [ ] 2.3 Write a failing test that delivery submits the rendered message
...
- [ ] 2.4 Write failing tests that a refused message and an unreachable server
...
- [ ] 2.5 Implement the sending half, taking the SMTP client as a factory
      argument so `tests/unit/` needs no mail server, and applying the fixed
      timeout from design.md
- [ ] 2.6 Add a test asserting the standard library's `email` package is what
      the adapter imports, despite the adapter's own package being named
      `email`

## 3. Email connection settings

- [ ] 3.1 Write failing tests: the SMTP settings resolve from the environment,
...
- [ ] 3.2 Write failing tests that a half-configured channel is a `ConfigError`
...
- [ ] 3.3 Implement the resolver beside the adapter, following
      `adapters/datadog/connection.py` and
      `adapters/sqlite_ledger/location.py`

## 4. Teams channel

- [ ] 4.1 Look up the current Workflows incoming-webhook envelope and Adaptive
...
- [ ] 4.2 Write a failing test that rendering a report produces the confirmed
...
- [ ] 4.3 Implement the pure rendering half in `adapters/teams/`
- [ ] 4.4 Write a failing test that delivery POSTs the rendered JSON to the
...
- [ ] 4.5 Write failing tests that a non-2xx response and a transport error
      each raise `NotifierError` carrying the status and response body
      (specs/notification - Teams rejects the post)
- [ ] 4.6 Implement the sending half over `urllib.request`, with the fixed
      timeout from design.md
- [ ] 4.7 Write failing tests that the webhook URL resolves from the
...
- [ ] 5.1 Write a failing test that a report is delivered to every configured
      channel exactly once (specs/notification - Every channel is attempted)
- [ ] 5.2 Write a failing test that a channel failing does not stop a later
      channel from being attempted (specs/notification - The first channel
      fails)
- [ ] 5.3 Write a failing test that delivery succeeds when at least one channel
...
- [ ] 5.4 Write a failing test that delivery raises `NotifierError` when every
...
- [ ] 5.5 Write a failing test that no channel is attempted more than once for
      one report (specs/notification - A failing channel is not retried in
      place)
- [ ] 5.6 Implement the fan-out notifier, itself satisfying the `Notifier` port
      so a caller cannot tell how many channels sit behind it

## 6. Channel resolution

- [ ] 6.1 Write failing tests that the active channels follow what the
      environment configured: email alone, Teams alone, and both
      (specs/config - Only one channel configured, Both channels configured)
- [ ] 6.2 Write a failing test that configuring no channel at all raises
...
- [ ] 6.3 Implement the resolver that assembles the fan-out notifier from the
      environment

## 7. Boundaries and closing out

- [ ] 7.1 Add `urllib.request` to the `forbidden_modules` list of the "Domain
...
- [ ] 7.2 Add an integration test in `tests/integration/` that sends a real
      ...
      service
- [ ] 7.3 Update the README with the notification environment variables and
      what activates each channel, and extend the "adding an adapter" guide to
      cover a notification channel
- [ ] 7.4 Run `uv run ruff check src tests`, `uv run ruff format --check src
