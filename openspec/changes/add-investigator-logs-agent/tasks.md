...
- [x] 4.1 `TriageDecision` gains `should_investigate`: due to be reported and
      attempts remaining — test a due incident with none spent, a due incident
      with attempts spent, and a suppressed incident
- [x] 4.2 Test that an incident whose report is overdue but whose attempts are
      spent is not investigated, however many runs handle it — the bound is on
      the incident, not on retries
- [x] 4.3 Test a bound of one disables retrying, and a bound below one is
      rejected as unusable configuration
- [x] 4.4 Test the counter clears on delivery, whatever the report carried, and
...
- [x] 6.4 Test a retry that fails again likewise delivers nothing, spends the
      second attempt, and still finishes the run unsuccessfully
- [x] 6.5 Test a retry that succeeds delivers the findings, stamps the incident
      as reported, and clears the counter
- [x] 6.6 Test a successful investigation whose delivery fails leaves the
      counter untouched, so the next run investigates again
- [x] 6.7 Test an investigation that completes and finds nothing notable is
      still delivered, because it is a result rather than a failure
- [x] 6.8 Test the last attempt failing delivers the alerts-only report, stamps
      the incident as reported, clears the counter, and still finishes the run
      unsuccessfully
- [x] 6.9 Test an incident with attempts spent and no report delivered yet is
...
- [x] 6.10 Test one group's investigation failure leaves the other groups their
      investigated reports, and the run finishes unsuccessfully

## 7. Config

- [x] 7.1 `Investigation` section on the `Config` port with `model` and
      `max_attempts` fields and documented defaults — test both defaults apply
      when the section is absent
- [x] 7.2 YAML loader resolves both keys, with `INVESTIGATION_MODEL` and
      `INVESTIGATION_MAX_ATTEMPTS` overriding the file — test file-only,
      environment-only, and both-set
- [x] 7.3 Test a credential-shaped key under `investigation` is inert:
      resolution proceeds as if it were absent
- [x] 7.4 Test `max_attempts` is resolved independently of the circuit
      breakers, and that changing one leaves the other alone

## 8. Datadog MCP adapter

- [x] 8.1 Add `google-adk` to `dependencies` in `pyproject.toml`, confirm
      `google`, `mcp`, and `pydantic` are in the import contract's
      `forbidden_modules`, and run the architecture test
- [x] 8.2 Derive the MCP endpoint from `DD_SITE` and build the auth headers
...
- [x] 9.7 The `Investigator` implementation: run the agent for one incident,
...
- [x] 10.4 Integration test of the retry arc across three runs against a real
