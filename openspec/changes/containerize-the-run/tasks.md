...
- [ ] 2.1 Add a `tests/integration/deployment/` package mirroring nothing under
...
- [ ] 2.2 Add a `conftest.py` there with the skip gate: a fixture that finds
      `docker` on the path and confirms the daemon answers, skipping with the
      reason named when either is absent
- [ ] 2.3 Add an `image` fixture resolving the tag from an environment variable
...
- [ ] 2.4 Confirm the gate works both ways: the suite passes with `docker`
      unavailable and reports the skips under `pytest -rs`

## 3. The image performs a run

- [ ] 3.1 **Red** — a test that runs the image with an environment holding no
      scope and asserts it exits non-zero naming `scope.owner`. It fails because
      there is no Dockerfile
- [ ] 3.2 **Green** — the smallest `Dockerfile` that passes it: a base image,
      the project installed, `ENTRYPOINT ["alert-triage"]` in exec form
- [ ] 3.3 **Refactor** — split into build and runtime stages, install with
      `uv sync --frozen --no-dev --no-editable` into a virtualenv the runtime
      stage copies, per `design.md`
- [ ] 3.4 **Red/Green** — a test that a fully configured run reaches the fetch
...
- [ ] 3.5 **Red/Green** — a test that no argument-free start is needed: assert
      the image declares no `CMD` that an appended argument would replace

## 4. What the image must not carry, and who it runs as

- [ ] 4.1 **Red** — a test asserting the built image contains no `.env`, no
      `config.yaml`, no `data/` and no virtualenv from the checkout. It fails
      against a build with no ignore file
- [ ] 4.2 **Green** — a `.dockerignore` covering environment files, the ledger
      directory, virtualenvs, caches, `.git`, and `.claude/`
- [ ] 4.3 **Red/Green** — a test that the development tooling is absent from the
      image (no pytest, ruff, mypy or import-linter in the runtime virtualenv)
- [ ] 4.4 **Red/Green** — a test that the run's process is owned by a non-root
      user; add the fixed UID/GID user to the Dockerfile to pass it
- [ ] 4.5 **Red/Green** — a test that a stale lockfile fails the build rather
...
- [ ] 5.1 **Red** — a test that a configured run given a mount at the image's
...
- [ ] 5.2 **Green** — set `ALERT_TRIAGE_LEDGER_PATH` as `ENV` to an absolute
      path under `/var/lib/alert-triage/`, create the directory, and `chown` it
      to the run's user. Declare no `VOLUME` — `design.md` says why
- [ ] 5.3 **Red/Green** — a test that a second container over the same mount
...
- [ ] 5.4 Confirm the mount is writable by the image's user under both a named
      volume and a bind mount, and record which needs host-side ownership work

## 6. The local repeat run

- [ ] 6.1 Add `compose.yaml`: the image, a named volume at the ledger location,
      `env_file: .env`, and nothing else it does not need
- [ ] 6.2 **Red/Green** — a test that two `docker compose run --rm` invocations
      share one ledger, so the documented local path proves the thing it exists
      for
- [ ] 6.3 Confirm no `[project.scripts]` entry was added for this — the decision
      and its reasoning are in `design.md`, and the entry point list stays the
      application's

## 7. The gate builds it

- [ ] 7.1 Add an image build step to `.github/workflows/ci.yml`, before the test
...
- [ ] 8.1 Add the container path to the README's "Running it": the `docker run`
...
- [ ] 8.2 Update the README's Setup so a reader with a container runtime and no
      checkout has a path, without displacing the checkout path
- [ ] 8.3 Note in `docs/configuration.md` that `ALERT_TRIAGE_LEDGER_PATH` has an
      image default distinct from the working-directory default, if that file
      states the default
- [ ] 8.4 Check whether `AGENTS.md` needs anything — it holds practices, and a
      new file kind that must not enter the image is arguably one

## 9. Before calling it done

- [ ] 9.1 `uv run ruff check src tests`, `uv run ruff format --check src tests`,
      `uv run mypy`, `uv run pytest` — all four green
- [ ] 9.2 Run the suite with `-rs` and confirm the container tests ran rather
