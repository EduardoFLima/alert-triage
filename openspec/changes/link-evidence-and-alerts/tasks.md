...
- [ ] 1.1 In `tests/unit/investigation/test_findings.py`, assert an
...
- [ ] 1.2 In `tests/unit/investigation/adapters/adk/test_items_are_read_out_of_any_result.py`,
...
- [ ] 2.1 In `tests/unit/investigation/adapters/adk/test_only_what_was_retrieved_is_citable.py`,
  ...
  `items_from`.
- [ ] 2.2 In the same file, assert the call-level `EvidenceItem` — the one an
...
- [ ] 2.3 In `tests/unit/investigation/adapters/adk/test_a_tool_result_reaches_the_model_checked.py`,
...
- [ ] 2.4 Confirm the model is still never shown an address: assert `_offered`
...
- [ ] 3.1 Write `tests/unit/investigation/adapters/datadog/test_evidence_is_addressable.py`
...
- [ ] 3.2 In the same file, assert a retrieval's address carries the query the
  tool was called with and the window it ran over, URL-encoded, so a reader
  lands on the search that produced the evidence.
- [ ] 3.3 Assert the builder is bound to the deployment's site, so an account on
  `datadoghq.eu` never receives a `datadoghq.com` address.

## 4. An alert links somewhere that opens

- [ ] 4.1 In `tests/unit/triage/adapters/datadog/test_alert_source.py`, assert a
...
- [ ] 4.2 Assert an event carrying no monitor id falls back to an Event Explorer
...
- [ ] 5.1 In `tests/unit/triage/domain/test_what_a_report_says.py`, assert a
...
- [ ] 5.2 Assert a long summary is still shortened while its address is rendered
  whole, which is the failure this change exists to fix.

## 6. Wiring

- [ ] 6.1 In `tests/unit/app/test_composition.py`, assert the investigator is
...
- [ ] 6.2 Run `uv run pytest tests/unit` and confirm `test_architecture.py`
...
- [ ] 7.1 In `tests/integration/triage/adapters/datadog/test_alert_source_live.py`,
  assert a built alert address answers rather than 404s. Credential-gated and
  skipped without them, like its neighbours.
- [ ] 7.2 In `tests/integration/investigation/adapters/datadog/test_logs_specialist_live.py`,
...
- [ ] 8.2 Update `README.md` only if a report's shape is shown there.
