...
**The flow is mermaid; the context map is hand-drawn ASCII.** The flow was
drafted through the mermaid MCP tool per `AGENTS.md`, the convention the
existing diagram was produced under. The context map breaks that rule
deliberately, because the thing being drawn is four hexagons and mermaid has no
hexagon: rendering a hexagonal architecture as rectangles makes the picture
argue against its own caption. `AGENTS.md` is amended to carry the exception
and its cost — ASCII is hand-aligned, so every later edit is column counting
mermaid would have done for you — rather than left saying "no hand-rolled
ASCII" while the README holds some.

The hexagons are generated by a short script rather than typed, so the borders
are symmetric by construction; two hand-drawn attempts before it were a
parallelogram and a shape with a stray vertical.
