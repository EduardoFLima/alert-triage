...
- [ ] 1.1 Read `src/alert_triage/app/pipeline.py` and write down the stages a
      run actually passes through, so the picture is drawn from the code rather
      than from the prose about it
- [ ] 1.2 Draft the flow through the mermaid MCP tool: alert in, report out,
      the ledger's decision visible as the reason a run may report nothing
- [ ] 1.3 Place it in the introduction, above the first heading, with at most a
      sentence of lead-in — the prose beneath it already says what it does
- [ ] 1.4 Render and confirm it is legible at GitHub's width without zooming

## 2. The bounded contexts

- [ ] 2.1 Draft the replacement through the mermaid MCP tool: one node per
...
- [ ] 2.2 Replace the existing diagram in `## Architecture` outright, keeping
      the surrounding prose, the source tree, and the note that both rules are
      enforced by `tests/unit/test_architecture.py`
- [ ] 2.3 Check the prose still reads correctly now that configuration is no
...
- [ ] 2.4 Render and confirm it is legible at GitHub's width without zooming

## 3. One section for getting it running, saying each setting once

- [ ] 3.1 Merge `## Setup` and `## Running it` into a single
      `## Getting started` with two subsections, `### Setup` and
      `### Running it`, keeping the prose of each intact for now
- [ ] 3.2 Demote `In a container` to `#### In a container` inside
      ...
      text
- [ ] 3.3 Move the two `cp` lines that copy `config.example.yaml` and
...
- [ ] 3.4 Confirm the other two passages named in `design.md` are still in
      ...
      the environment
- [ ] 3.5 Leave `Configure` holding the minimum a first run needs, a one-line
...
- [ ] 3.6 Diff the `Running it` environment list against
      `docs/configuration.md`; move anything only the README carries into that
      reference before removing it
- [ ] 3.7 Reduce that list to what a run needs plus a pointer to the reference
- [ ] 3.8 Re-read the merged section end to end: with the two halves adjacent,
      cut whatever else now says the same thing twice
- [ ] 3.9 Confirm the container subsection is still reachable as the second
...
- [ ] 4.1 Add the list convention to `AGENTS.md` under `Looking things up`,
...
- [ ] 4.2 Add the second convention beside it: documentation does not restate
      ...
      commands to run
- [ ] 4.3 Confirm `CLAUDE.md` and `GEMINI.md` still resolve to the edited file
      — they are symlinks, so this should need no action, which is the point
- [ ] 4.4 Rewrite `Extending it`: the two kinds of extension become two
...
- [ ] 4.5 Apply the list convention to the other place in `README.md` whose
...
- [ ] 4.6 Trim `Development` under the second convention: delete the "you do
...
- [ ] 4.7 Confirm `Development` still carries all four gate commands and every
      pytest selection — only the explaining goes
- [ ] 4.8 Confirm nothing was reshaped that was not enumerating, and nothing
      was cut that only the README said

## 5. Move the container detail to its own document

- [ ] 5.1 Create `docs/containerized.md` and move into it the five-row flag
...
- [ ] 5.2 Reduce `In a container` to the `docker build` and `docker run`
...
- [ ] 5.3 Keep the README's `docker run` to the two flags a run cannot do
...
- [ ] 5.4 Re-point `docs/configuration.md`'s `../README.md#in-a-container`
      link at the new document, where the mount detail now lives
- [ ] 5.5 Confirm the durable-mount warning survived the move to the README
      side, since a reader who never follows the link must still meet it

## 6. Confirm

- [ ] 6.1 Check every link and anchor the edited sections use still resolves
- [ ] 6.2 Read the rendered README start to finish as someone who has not seen
      the codebase, and confirm each new scenario in the delta spec holds
- [ ] 6.3 Confirm the extension guide still tells a contributor both kinds
      apart and still points to the guide, which is what the spec requires of
      it and what the rewrite must not lose
- [ ] 6.4 Run the four gate commands — they cover no documentation, but a green
