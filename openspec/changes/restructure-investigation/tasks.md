...
- [ ] 1.3 `Findings.retrieval_failures`, defaulting to empty, with a `complete`
      ...
      notable was found
- [ ] 1.4 Confirm `tests/unit/test_architecture.py` still passes: the domain
      imports the standard library and nothing else

## 2. The shallow normaliser

- [ ] 2.1 Items from a result that is a bare list — test each entry becomes one
      `EvidenceItem` in order
- [ ] 2.2 Items from a result wrapping its entries in a recognised envelope key
      — test the entries are found and the envelope is not itself an item
- [ ] 2.3 A result with no readable items — test it yields no items rather than
      an error, so a tool the project has never seen degrades to a citable
      aggregate
- [ ] 2.4 Normalising one item: an instant read where the payload offers one and
...
- [ ] 3.1 `Retrieved` retains a tool result under `call-N` — test two calls in
      one investigation get distinct identifiers and the results are kept
      verbatim
- [ ] 3.2 Items within a retained call are addressable as `call-N/item-M` —
      test the identifiers the model is shown are the ones that resolve
- [ ] 3.3 Resolving a call-level citation yields the call's evidence — test a
      finding about an aggregate keeps its evidence
- [ ] 3.4 Resolving an item-level citation yields that item — test a pattern
      finding keeps exactly the items it cited
- [ ] 3.5 A citation naming a call or item never retrieved resolves to nothing
      ...
      grain
- [ ] 3.6 Test a fresh `Retrieved` per investigation: an identifier from an
      earlier investigation resolves to nothing

## 4. The gate: a failed retrieval is not silence

- [ ] 4.1 The evidence callback retains a successful result and hands the model
      the citable form instead of the raw one — test the model never sees the
      raw payload and always sees the identifiers
- [ ] 4.2 A result carrying `isError` is recorded as a retrieval failure and
...
- [ ] 4.3 A result carrying an `error` key is treated identically — test ADK's
      converted-exception shape takes the same path as a server-side error
- [ ] 4.4 A result that cannot be read at all is a retrieval failure, not an
      empty one — test an unreadable payload is recorded and refused rather
      than passed through as nothing found
- [ ] 4.5 Failures accumulate on the investigation alongside successful
      evidence — test two failures and three successes are all recorded, and
      the successes stay citable
- [ ] 4.6 `before_tool_callback` is registered and logs the call without
...
- [ ] 5.1 The declaration value (name, signal, instruction, output schema,
...
- [ ] 5.2 Build an agent from a declaration — test the agent's name,
      instruction, and output schema come from the declaration and nothing
      else does
- [ ] 5.3 The toolset built for a declaration is filtered to the tool names it
      named — test a tool the declaration did not name is not exposed, and
      that the endpoint asks for the declared toolsets
- [ ] 5.4 A declaration naming its own model beats the default; one naming none
      takes the default — test both against a crew of two declarations
- [ ] 5.5 Deployment facts are supplied, not declared — test the same
      declaration built against two different endpoints and credentials is
      unchanged
- [ ] 5.6 The connection params carry explicit connect and read bounds — test
...
- [ ] 6.1 Rewrite the logs instruction: the platform's log tools and query
...
- [ ] 6.2 The logs declaration names its signal, its toolset, and its log tools
      — test the crew contains it and that it reaches no tool outside its
      declaration
- [ ] 6.3 The output schema cites at both grains — test a payload citing a call,
      one citing items, and one citing both all build findings

## 7. The investigator drives declarations

- [ ] 7.1 The coordinator runs every declaration in the crew and concatenates
...
- [ ] 7.2 Some retrievals failed, findings produced → findings returned marked
      incomplete — test against a stub model
- [ ] 7.3 Every retrieval failed → `InvestigatorError`, not an empty result —
      test this is the outcome even when the stub model reports findings
      confidently
- [ ] 7.4 No retrieval attempted → an ordinary complete result — test a model
      that chose not to look is not a failure
- [ ] 7.5 A specialist that errors outright is still a failed investigation —
...
- [ ] 8.1 Render `EvidenceItem` in the investigated report — test the body
      carries each finding's summary lines and instants
- [ ] 8.2 An incomplete investigation says so — test the body states the
      evidence gathered was incomplete, alongside the findings it did produce
- [ ] 8.3 A complete investigation carries no such note — test both the
      notable and the nothing-notable cases
- [ ] 8.4 Test the incompleteness note is independent of the "investigation
      could not complete" report, which still covers the spent-attempts case

## 9. Per-specialist model configuration

- [ ] 9.1 `investigation.specialists.<name>.model` in the config port and the
      YAML loader — test a specialist named there runs on that model and every
      other on `investigation.model`
- [ ] 9.2 Test the section is optional and an unknown specialist name is
      refused by name, like any key the schema has never heard of
- [ ] 9.3 Test the environment override path reaches a per-specialist model
      like every other behavior value

## 10. Composition and removal

- [ ] 10.1 The composition root builds the crew from the declarations and
      supplies endpoint, headers, and default model — test it names no tool
- [ ] 10.2 Delete `ports/observability_platform.py`, `DatadogMcpPlatform` and
      its translation, the `search_logs` tool wrapper, and `LogRecord` —
      confirm nothing imports them and the whole suite is green
- [ ] 10.3 Confirm `tests/unit/test_architecture.py` passes: `mcp` is no longer
