...
- [ ] 1.1 Change the failing expectation in `tests/unit/test_triage.py`: a
...
- [ ] 1.2 Add a failing test that an incident already reported keeps its
      ...
      delivered)
- [ ] 1.3 Make them pass by removing the `reported(now)` call from `triage()`,
...
- [ ] 2.1 Write a failing test that a report built from an incident names the
      ...
      and alerts)
- [ ] 2.2 Write a failing test that the body states the alerts have not been
      investigated (specs/triage-run - The report does not pretend to be
      triage)
- [ ] 2.3 Write a failing test covering an alert with no title and no link, so
      the builder handles the defaults `Alert` allows
- [ ] 2.4 Implement the builder in `domain/report.py` beside `TriageReport`,
      stdlib-only, producing a valid single-line subject

## 3. The run

- [ ] 3.1 Write a failing test that a run with one group and nothing on record
...
- [ ] 3.2 Write failing tests for the empty run and the multi-service run:
...
- [ ] 3.3 Write a failing test that the run asks the alert source for alerts
      from the run's instant minus the configured lookback (specs/triage-run -
      A run fetches over the configured lookback)
- [ ] 3.4 Write a failing test that the same supplied instant reaches the
...
- [ ] 4.3 Write a failing test that a run with no failure finishes
      successfully (specs/triage-run - Every group succeeds)
- [ ] 4.4 Write a failing test that each recorded failure names the stage and
      the service it concerns (specs/triage-run - A failure is diagnosable)
- [ ] 4.5 Implement `RunOutcome` and the per-group containment in
