...
- [x] 1.1 Write failing tests for an `Incident` carrying a generated
      ...
      generated identity)
- [x] 1.2 Write a failing test that absorbing further alerts yields an incident
...
- [x] 1.3 Implement `domain/incident.py` to pass those tests, stdlib-only

## 2. Continuation rule

- [x] 2.1 Write a failing test that a group sharing an alert identifier with a
      ...
      alerts)
- [x] 2.2 Write a failing test that a group mixing recorded and unrecorded
      alerts continues the incident and absorbs only the new ones
      (specs/triage-ledger - A firing incident produces new alerts)
- [x] 2.3 Write a failing test that a group sharing no alert identifier but
...
- [x] 2.4 Write failing tests that a group beyond that window opens a new
      ...
      distinguishable)
- [x] 2.5 Implement the continuation rule in `domain/triage.py`, taking the id
      factory as an argument so tests supply deterministic identifiers

## 3. Cooldown decision

- [x] 3.1 Write a failing test that a newly opened incident is reported
      (specs/triage-ledger - A newly opened incident)
- [x] 3.2 Write failing tests that a continuation inside the cooldown is
...
- [x] 3.3 Write a failing test that the cooldown is measured from the most
      recent report, not the first (specs/triage-ledger - The cooldown restarts
      on each report)
- [x] 3.4 Write a failing test that an incident inside its cooldown does not
      suppress a separate incident on the same service (specs/triage-ledger -
      Suppression is per incident, not per service)
- [x] 3.5 Write failing tests that the decision is evaluated against a supplied
...
- [x] 3.6 Extend `domain/triage.py` with the report-versus-suppress decision to
      pass those tests, returning the resulting incident alongside the verdict

## 4. Config additions

- [x] 4.1 Write failing tests: the re-notify cooldown resolves to its
      ...
      Re-notify cooldown)
- [x] 4.2 Add a `ReNotify` section to the `Config` port and resolve it in the
      YAML/env adapter, reusing the existing `section.key` → `SECTION_KEY`
      mapping, with the two-day default from `docs/vision.md`
- [x] 4.3 Write failing tests: the ledger retention period resolves to a
...
- [x] 4.4 Add a `Ledger` section to the `Config` port carrying the retention
...
- [x] 4.5 Write failing tests: the ledger storage location resolves from the
...
- [x] 4.6 Resolve the storage location alongside the other connection settings
      slice 2 established, not in the YAML-backed behavior config

## 5. TriageLedger port

- [x] 5.1 Write failing tests for the port's shape: synchronous, retrieving the
      ...
      process)
- [x] 5.2 Define `ports/triage_ledger.py` with a `TriageLedgerError` beside it,
      mirroring how `AlertSourceError` sits beside `AlertSource`

## 6. SQLite adapter — storage

- [x] 6.1 Write a failing test that a recorded incident is read back with its
...
- [x] 6.2 Write a failing test that a service with nothing on record returns an
      ...
      is not a failure)
- [x] 6.3 Write a failing test that timestamps round-trip as timezone-aware UTC
      values, so no naive datetime escapes the adapter into cooldown arithmetic
- [x] 6.4 Implement `adapters/sqlite_ledger/` taking an injected
      `sqlite3.Connection` and ensuring its own schema on construction, to pass
      those tests

## 7. Closing, retention, and failure

- [x] 7.1 Write failing tests that an incident past both the grouping window
...
- [x] 7.2 Write failing tests that a closed incident neither continues into a
...
- [x] 7.3 Write a failing test that retrieval offers only open incidents to a
...
- [x] 7.4 Write a failing test that the instant an incident closed is stamped
      once and read back unchanged, rather than recomputed — so retuning the
      cooldown does not move it
- [x] 7.5 Write failing tests that a closed incident within the retention
...
- [x] 7.6 Write a failing test that a retained closed incident leaves the
...
- [x] 7.7 Write failing tests that a failed read and a failed write each raise
...
- [x] 7.8 Implement closing, retention-bounded deletion on write, and
      vendor-exception translation at the adapter boundary to pass those tests

## 8. Documentation

- [x] 8.1 Update `docs/vision.md`'s re-notification section with the
      ...
      the slice 3 entry
- [x] 8.2 Update the README's setup section with the cooldown and retention
...
- [x] 9.1 Add an integration test (`tests/integration/`) that records an
      incident to a real file on disk and reads it back through a second
      connection, covering what an in-memory database cannot
- [x] 9.2 Run `uv run ruff check src tests`, `uv run ruff format --check src
      tests`, `uv run mypy`, and `uv run pytest`; all four pass
- [x] 9.3 Confirm no domain or ports module imports the adapter, and that the
