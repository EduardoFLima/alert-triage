...
- [x] 1.1 Add `datadog-api-client` to the project dependencies, and in the same
...
- [x] 1.2 Confirm the architecture test still passes and would fail on a stray
...
- [x] 2.1 Write failing tests for an `Alert` carrying a source identifier,
      title, and link alongside service and timestamp (specs/alert-grouping -
      Alert entity)
- [x] 2.2 Write a failing test that two alerts differing only in the added
      fields still group together (specs/alert-grouping - Alert entity,
      "Grouping ignores the added fields")
- [x] 2.3 Extend `Alert` to pass those tests, leaving the grouping logic
      untouched

## 3. Config additions

- [x] 3.0 Rename `scope.datadog_team` to `scope.owner` and `SCOPE_DATADOG_TEAM`
...
- [x] 3.1 Write failing tests: the ingestion lookback resolves to its
      ...
      window)
- [x] 3.2 Write failing tests: ingestion's request timeout and retry bound
...
- [x] 3.2a Extend the `Config` port and the YAML/env adapter with the lookback
...
- [x] 3.3 Write failing tests: a site or credential key written into
      ...
      connections)
- [x] 3.4 Write failing tests: the site and credentials resolve from the
...
- [x] 3.5 Implement connection-settings resolution to pass those tests, as a
      type separate from the YAML-backed behavior config so the two cannot be
      confused at a call site

## 4. AlertSource port

- [x] 4.1 Write failing tests for the `AlertSource` port's shape: a
      ...
      vocabulary)
- [x] 4.2 Define the `AlertSource` port in `ports/`, with an
      `AlertSourceError` beside it mirroring how `ConfigError` sits beside
      `Config`

## 5. Datadog adapter — translation

- [x] 5.1 Write failing tests translating a canned event payload into an
      ...
      Alert entities)
- [x] 5.2 Write a failing test that an event carrying no `service:` tag is
      excluded while its siblings are returned (specs/alert-ingestion -
      Alerts without a resolvable service are excluded)
- [x] 5.3 Implement the adapter's event → `Alert` translation, taking an
      injected API client, to pass those tests

## 6. Datadog adapter — querying

- [x] 6.1 Write a failing test asserting the request translates the configured
...
- [x] 6.2 Write a failing test that a result spanning several cursor pages
      returns alerts from every page (specs/alert-ingestion - Complete results
      across pagination)
- [x] 6.3 Write a failing test that a run matching nothing succeeds with no
      alerts (specs/alert-ingestion - No alerts is a valid result)
- [x] 6.4 Implement scoped, time-bounded querying with cursor pagination to
      pass those tests

## 7. Datadog adapter — failure and bounds

- [x] 7.1 Write failing tests that a rejected credential and a failure
...
- [x] 7.2 Write failing tests that the client is constructed with ingestion's
...
- [x] 7.3 Implement vendor-exception translation at the adapter boundary and
      map ingestion's two request bounds onto the client's timeout and retry
      configuration

## 8. Documentation

- [x] 8.1 Update `docs/vision.md`: the `AlertSource` port line and the slice 2
      entry now describe a REST adapter, with a sentence on why MCP stays with
      `ObservabilityPlatform`
- [x] 8.1a Update `docs/vision.md`'s config section for the `scope.owner`
      rename, including the `SCOPE_DATADOG_TEAM` example used to illustrate
      the env-var naming convention
- [x] 8.2 Update the README's setup section: the lookback config key, the site
...
- [x] 8.3 Update `docs/vision.md`'s config section, which currently says "any
...
- [x] 9.1 Add an integration test (`tests/integration/`) that exercises the
...
- [x] 9.3 Confirm no domain or ports module imports the Datadog SDK, and that
