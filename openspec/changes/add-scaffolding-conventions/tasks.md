...
- [ ] 1.1 Initialize the project with uv: create `pyproject.toml` with `[project]`
  metadata (name `alert-triage`, pinned `requires-python`) and a `src/` layout
  build target; commit the generated `uv.lock`
- [ ] 1.2 Add dev dependencies only — ruff, mypy, pytest, pytest-cov,
  import-linter — leaving runtime dependencies empty until the slice that needs
  them
- [ ] 1.3 Configure ruff (lint rule selection + formatter) and mypy (strict) in
  `pyproject.toml`; confirm `uv run ruff check`, `uv run ruff format --check`,
  and `uv run mypy` all execute cleanly on the empty tree
- [ ] 1.4 Add `.gitignore` covering the venv, caches, coverage output, and build
  artifacts

## 2. Package layout

- [ ] 2.1 Create `src/alert_triage/` with `domain/`, `ports/`, `adapters/`, and
  `app/` packages, each with an `__init__.py` whose docstring states that layer's
  allowed dependency direction
- [ ] 2.2 Create the adapter subpackages reserved by `docs/vision.md` —
  `adapters/datadog/`, `adapters/adk/`, `adapters/email/`, `adapters/teams/` —
  as empty packages
- [ ] 2.3 Verify the package imports from an installed environment (`uv run python
  -c "import alert_triage"`), confirming the `src/` layout is wired correctly

## 3. Test harness

- [ ] 3.1 Create `tests/unit/` and `tests/integration/` with a root `conftest.py`
  that applies the `unit` / `integration` marker automatically based on a test's
  directory
- [ ] 3.2 Configure pytest in `pyproject.toml`: testpaths, marker registration,
  strict markers, and coverage reporting
- [ ] 3.3 Add a smoke test asserting the package imports, and confirm
  `uv run pytest -m unit` selects only the unit directory while `uv run pytest`
  runs everything

## 4. Architecture boundary enforcement

- [ ] 4.1 Write failing tests first: temporary fixture modules that violate the
...
- [ ] 4.2 Configure import-linter in `pyproject.toml` — a `layers` contract
  ...
  and `ports`
- [ ] 4.3 Add `tests/unit/test_architecture.py` that runs the contracts and fails
  with the offending module and import named in the output
- [ ] 4.4 Confirm both directions: the violation fixtures fail the check, then
  remove them and confirm the clean tree passes
  (spec: "Hexagonal import boundary is machine-enforced")

## 5. Agent instructions

- [ ] 5.1 Write canonical `AGENTS.md` covering practices only — clean code, TDD
...
- [ ] 5.2 Create `CLAUDE.md` and `GEMINI.md` as git symlinks to `AGENTS.md` and
  verify git recorded them as symlinks, not copies
- [ ] 5.3 Add `tests/unit/test_agent_instructions.py` asserting each
