...
def test_the_endpoint_is_derived_from_the_default_site() -> None:
    assert mcp_endpoint(_connection()) == "https://mcp.datadoghq.com/v1/mcp"
...
def test_the_endpoint_follows_a_non_default_site() -> None:
    ...
    assert (
        mcp_endpoint(_connection("datadoghq.eu")) == "https://mcp.datadoghq.eu/v1/mcp"
    )
...
def test_the_endpoint_asks_for_no_toolset_of_its_own() -> None:
    ...
    assert "toolsets" not in mcp_endpoint(_connection())
